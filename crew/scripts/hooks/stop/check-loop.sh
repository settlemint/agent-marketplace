...
if [ "$ITERATION" -ge "$MAX_ITERATIONS" ]; then
	...
	echo "  - Continue manually: Skill(skill: \"crew:work\")"
...
echo "  Run /compact now to clear context before continuing."
echo "  All progress is preserved in:"
echo "    - Task files: .claude/branches/$SAFE_BRANCH/tasks/"
...
echo "$PROMPT"
echo ""
echo "CRITICAL RULES (apply after compaction):"
echo "  - NEVER run CI commands directly (bun run test, etc) - use haiku test-runner agent"
echo "  - Max 6 agents per batch, launched in SINGLE message"
echo "  - Agents do NOT run tests - test-runner handles this"
echo "  - Mark tasks complete IMMEDIATELY (rename *-pending-* → *-complete-*)"
echo ""
echo "PROGRESS CHECK:"
echo "  1. Load pending tasks from .claude/branches/$SAFE_BRANCH/tasks/*-pending-*.md"
echo "  2. Execute remaining tasks in batches (max 6 per batch)"
echo "  3. Run test-runner agent (haiku) after each batch"
echo "  4. Create fix tasks for any failures"
echo "  5. When ALL tasks complete and CI passes:"
echo "     Output: <promise>$COMPLETION_PROMISE</promise>"
echo ""
echo "STRICT REQUIREMENT:"
