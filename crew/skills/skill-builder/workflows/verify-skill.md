...
mcp__context7__getContext({
  query: "What are the current {relevant-topic} patterns?",
  libraryId: "{library-id}"  // e.g., "/stripe/stripe-node"
})
```
...
```
mcp__context7__getContext({
  query: "What are the current {specific-api} patterns and best practices?",
...
mcp__context7__searchLibrary({
  query: "What I need from {library-name}",
  libraryName: "{library-name}",
});

// Get documentation (main method)
mcp__context7__getContext({
  query: "How do I use {specific-feature}?",
