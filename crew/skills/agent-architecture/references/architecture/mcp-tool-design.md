...
export const serverName = createSdkMcpServer({
  ...
  tools: [
    ...
    tool(
      "read_item",
      "Read an item by key",
      { key: z.string().meta({ description: "Item key" }) },
      async ({ key }) => {
    ...
    tool(
      ...
      "List all items, optionally filtered",
      {
        prefix: z
          .string()
          .optional()
          .meta({ description: "Filter by key prefix" }),
        limit: z.number().default(100).meta({ description: "Max items" }),
      },
      async ({ prefix, limit }) => {
    ...
    tool(
      ...
      {
        key: z.string().meta({ description: "Item key" }),
        value: z.any().meta({ description: "Item data" }),
      },
      async ({ key, value }) => {
    ...
    tool(
      ...
      "Store a feedback item",
      {
        item: z
          ...
          .meta({ description: "Feedback item" }),
      },
      async ({ item }) => {
