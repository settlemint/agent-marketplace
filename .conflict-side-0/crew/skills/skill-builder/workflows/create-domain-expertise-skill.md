...
```javascript
// Get documentation with natural language query
mcp__context7__getContext({
  query: "Complete guide to {library-name} patterns and best practices",
  libraryId: "{library-id}", // e.g., "/reactjs/react.dev"
});
```
...
options: - label: "Build new {thing}"
description: "Create a new {thing} from scratch" - label: "Debug {thing}"
description: "Fix issues with an existing {thing}" - label: "Add feature"
description: "Extend functionality of existing {thing}" - label: "Write tests"
description: "Create or run tests for {thing}"
multiSelect: false
